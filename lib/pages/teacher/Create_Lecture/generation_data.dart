...
@override
  ...
  checkingPastQrData() async {
    List extraQrDocId = [];
    final QuerySnapshot querySnapshot = await FirebaseFirestore.instance.collection("class").document(globals.classCode).collection("lectureID_qrCode").getDocuments();
    ...
    if (querySnapshot.documents.length > 1) {
      ...
      for (int i = 1; i < extraQrDocId.length; i++) {
        ...
        FirebaseFirestore.instance.collection("class").document("${globals.classCode}").collection("lectureID_qrCode").document(extraQrDocId[i]).delete();
      }
     createQrDocument();
    }
    else{
  ...
  Future createQrDocument() async {
    ...
    DocumentReference qrId = await FirebaseFirestore.instance.collection("class")
                      ...
                      if (selectedCourseCode != null &&
                        ...
                        for (int i = 0; i <
                          ...
                          globals.studentId.insert(
                              i, "${querySnapshot.docs[i].documentID}");
                        }
...
//                            globals.startAddingStudents = 0;
  ...
  getClassDetails() async {
    ...
    DocumentSnapshot snapshot = await FirebaseFirestore.instance.collection('class')
