  ...
  Future _saveAttendanceToFile() async {
    for (int i = 0; i < globals.studentDocumentId.length; i++) {
      ...
      DocumentSnapshot snapshot = await FirebaseFirestore.instance
          ...
          .collection("attendance")
          .doc("${globals.studentDocumentId[i]}")
          .get();
      if (snapshot.data == null) {
        debugPrint("No data in student id ${globals.studentDocumentId[i]} ");
      } else {
        globals.attendanceDetails.putIfAbsent(
            "${snapshot.data['id']}", () => "${snapshot.data['attendance']}");
      }
    }
    Directory appDocDir = await getApplicationDocumentsDirectory();

    final file = File(
        "${appDocDir.path}/${globals.classCode}_${globals.courseCode}_${new DateTime.now()}.txt");
          ...
          children: <Widget>[
            new ListTile(
              ...
              title: new FlatButton(
                  ...
                  child: new Text(
                      'Click here to Save the file to downloads folder')),
            ),
          ],
        )));
  }
